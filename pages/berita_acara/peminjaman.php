...
$stmt = $pdo->prepare("
    SELECT p.*, 
           a.nama_aset, a.kode_aset, a.kondisi as kondisi_awal, a.jumlah as stok_aset, a.sumber_dana,
           k.nama_kategori,
...
$nomorBA = 'BA.PINJAM/' . sprintf('%03d', $data['id']) . '/MAN.2.HSU/' . date('Y', strtotime($data['tanggal_pinjam']));
$tglPinjamText = tglIndo($data['tanggal_pinjam']);
$hariPinjamText = hariIndo($data['tanggal_pinjam']);
$tglKembaliRencanaText = tglIndo($data['tanggal_kembali_rencana']);
...
if (isset($_GET['download']) && $_GET['download'] === 'pdf') {
    ...
    ob_start();
    include __DIR__ . '/_template_peminjaman.php';
    $htmlContent = ob_get_clean();
    
    $dompdf->loadHtml($htmlContent);
    $dompdf->setPaper('A4', 'portrait');
    $dompdf->render();
    $dompdf->stream('Berita_Acara_Peminjaman_' . $data['id'] . '.pdf', ['Attachment' => true]);
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Berita Acara Peminjaman Aset #<?= $data['id'] ?> - MAN 2 HSU</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        ol.klausul-list { margin-left: 28px; margin-bottom: 10px; font-size: 10pt; line-height: 1.4; }
        ol.klausul-list li { margin-bottom: 4px; }
        
        .table-ttd { width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 10pt; }
...
<div class="no-print-bar">
    <div style="font-family:sans-serif; font-size:14px; display:flex; align-items:center; gap:8px;">
        ...
        <span><strong>Berita Acara Peminjaman Aset</strong> (No: <?= $nomorBA ?>)</span>
    </div>
...
<div class="page-sheet">
    ...
    <!-- Judul Berita Acara -->
    <div class="ba-title">BERITA ACARA SERAH TERIMA PEMINJAMAN ASET</div>
    <div class="ba-number">Nomor: <?= $nomorBA ?></div>

    <div class="ba-content">
        <p>Pada hari ini <strong><?= $hariPinjamText ?></strong> tanggal <strong><?= $tglPinjamText ?></strong>, bertempat di Madrasah Aliyah Negeri 2 Hulu Sungai Utara, kami yang bertanda tangan di bawah ini:</p>
        ...
        <table class="table-pihak">
            ...
            <tr>
                <td colspan="3" style="padding-top:4px; font-style:italic;">Selaku pihak yang menyerahkan barang, selanjutnya disebut <strong>PIHAK PERTAMA</strong>.</td>
            </tr>
        </table>
        ...
        <table class="table-pihak" style="margin-top:6px;">
            ...
            <tr>
                <td class="lbl">&nbsp;&nbsp;&nbsp;&nbsp;No. Telepon/HP</td>
                <td class="sep">:</td>
                <td><?= htmlspecialchars($data['telp_guru'] ?: '-') ?></td>
            </tr>
            <tr>
                <td colspan="3" style="padding-top:4px; font-style:italic;">Selaku pihak yang menerima dan meminjam barang, selanjutnya disebut <strong>PIHAK KEDUA</strong>.</td>
            </tr>
        </table>

        <p style="margin-top:8px;">PIHAK PERTAMA menyerahkan barang inventaris madrasah kepada PIHAK KEDUA, dan PIHAK KEDUA menyatakan telah menerima barang tersebut dalam kondisi baik dan lengkap dengan rincian sebagai berikut:</p>
        ...
        <table class="table-barang">
            ...
            <tbody>
                <tr>
                    ...
                    <td style="text-align:center;"><?= htmlspecialchars($data['kondisi_awal']) ?></td>
                    <td><?= htmlspecialchars($data['lokasi_awal'] ?? '-') ?></td>
                </tr>
            </tbody>
        </table>

        <p class="no-indent"><strong>Ketentuan dan Syarat Peminjaman:</strong></p>
        <ol class="klausul-list">
            <li>Barang inventaris di atas dipinjam untuk keperluan: <em>"<?= htmlspecialchars($data['keterangan'] ?: 'Kegiatan operasional / pembelajaran madrasah') ?>"</em>.</li>
            <li>Jangka waktu peminjaman disepakati mulai tanggal <strong><?= $tglPinjamText ?></strong> sampai dengan batas waktu tanggal <strong><?= $tglKembaliRencanaText ?></strong>.</li>
            <li>PIHAK KEDUA berkewajiban merawat, menjaga keutuhan, dan tidak memindahtangankan barang tersebut kepada pihak lain tanpa izin tertulis.</li>
            <li>Apabila terjadi kerusakan atau kehilangan akibat kelalaian selama masa peminjaman, PIHAK KEDUA bersedia bertanggung jawab sesuai dengan peraturan yang berlaku di MAN 2 HSU.</li>
        </ol>

        <p>Demikian Berita Acara Serah Terima Peminjaman ini dibuat dengan sebenarnya dalam rangkap secukupnya untuk dipergunakan sebagaimana mestinya.</p>
    </div>
