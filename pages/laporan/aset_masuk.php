...
<div class="page-header">
    <div><h2><i class="fas fa-arrow-right-to-bracket"></i> Laporan Aset Masuk</h2></div>
    <div class="btn-group no-print">
        <a href="export_pdf.php?type=aset_masuk&start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>" class="btn btn-danger"><i class="fas fa-file-pdf"></i> Cetak PDF</a>
        <a href="export_pdf.php?type=aset_masuk&start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>&view=1" target="_blank" class="btn btn-info"><i class="fas fa-eye"></i> Lihat PDF</a>
        <a href="export_excel.php?type=aset_masuk&start_date=<?= urlencode($startDate) ?>&end_date=<?= urlencode($endDate) ?>" class="btn btn-success"><i class="fas fa-file-excel"></i> Export Excel</a>
    </div>
</div>
...
<div class="card animate-fadeInUp"><div class="card-header"><h3>Aset Masuk (<?= count($data) ?> item)</h3></div><div class="card-body"><div class="table-wrapper">
    <table><thead><tr><th>No</th><th>Tgl Masuk</th><th>Kode</th><th>Nama Aset</th><th>Kategori</th><th>Lokasi</th><th>Jumlah</th><th>Sumber Dana</th><th>Nilai</th></tr></thead>
    <tbody>
        <?php $total = 0; foreach ($data as $i => $a): $total += $a['nilai_perolehan'] * $a['jumlah']; ?>
        <tr>
            <td><?= $i+1 ?></td>
            ...
            <td><?= $a['jumlah'] ?></td><td><?= htmlspecialchars($a['sumber_dana'] ?? '-') ?></td>
            <td style="text-align:right;"><?= formatRupiah($a['nilai_perolehan'] * $a['jumlah']) ?></td>
        </tr>
        <?php endforeach; ?>
        <tr style="font-weight:700;"><td colspan="8" class="text-right">TOTAL</td><td style="text-align:right;"><?= formatRupiah($total) ?></td></tr>
    </tbody></table>
</div></div></div>
